...
std::vector<double> LH(std::vector<double> sol, int flux)
{
    ...
    {
        wright[RHO]=Numeric_flux(sol,1,flux)[RHO][j];
       
        left[RHO]=Numeric_flux(sol,-1,flux)[RHO][j];

        wright[U]=Numeric_flux(sol,1,flux)[U][j];
        left[U]=Numeric_flux(sol,-1,flux)[U][j];

        wright[E]=Numeric_flux(sol,1,flux)[E][j];
        left[E]=Numeric_flux(sol,-1,flux)[E][j];
